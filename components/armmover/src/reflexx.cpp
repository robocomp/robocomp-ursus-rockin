...
Reflexx::Reflexx(RoboCompJointMotor::JointMotorPrx jointmotor_proxy, const QList< QPair<QString, float> > &points)
{
	...
	{
		IP->CurrentPositionVector->VecData[i] = points[i].second;
...
void Reflexx::run()
{
	...
	for(int i=0; i<NUMBER_OF_DOFS; i++)
	{
		IP->SelectionVector = false;		//WATCH THIS
	}
