...
class FaceRecognitionComp : public RoboComp::Application
{
private:
...
void FaceRecognitionComp::initialize()
{
...
int FaceRecognitionComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
    	...
    	// Server adapter creation and publication
		Ice::ObjectAdapterPtr adapterFaceRecognition = communicator()->createObjectAdapter("FaceRecognitionComp");
		FaceRecognitionI *facerecognition = new FaceRecognitionI(worker);
		adapterFaceRecognition->add(facerecognition, communicator()->stringToIdentity("facerecognition"));

		adapterFaceRecognition->activate();
