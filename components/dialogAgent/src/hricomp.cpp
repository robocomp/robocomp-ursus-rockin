...
class hriComp : public RoboComp::Application
{
private:
...
int hriComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		asrcomprehension_proxy = ASRComprehensionPrx::uncheckedCast( communicator()->stringToProxy( getProxyString("ASRComprehensionProxy") ) );
	}
	catch(const Ice::Exception& ex)
	{
	...
	try
	{
    	...
    	// Server adapter creation and publication
    	Ice::ObjectAdapterPtr ASRCommand_adapter = communicator()->createObjectAdapter("ASRCommandTopic");
    	ASRCommandPtr asrcommandI_ = new ASRCommandI(worker);
    	Ice::ObjectPrx asrcommand_proxy = ASRCommand_adapter->addWithUUID(asrcommandI_)->ice_oneway();
    	IceStorm::TopicPrx asrcommand_topic;
    	if(!asrcommand_topic){
	    	try {
	    		asrcommand_topic = topicManager->create("ASRCommand");
	    	}
	    	catch (const IceStorm::TopicExists&) {
	    	  	//Another client created the topic
	    	  	try{
	       			asrcommand_topic = topicManager->retrieve("ASRCommand");
	    	  	}catch(const IceStorm::NoSuchTopic&){
	    	  	  	//Error. Topic does not exist
				}
	    	}
	    	IceStorm::QoS qos;
	      	asrcommand_topic->subscribeAndGetPublisher(qos, asrcommand_proxy);
    	}
    	ASRCommand_adapter->activate();
    	// Server adapter creation and publication
