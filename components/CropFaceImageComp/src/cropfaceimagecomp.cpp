...
class CropFaceImageComp : public RoboComp::Application
{
private:
...
void CropFaceImageComp::initialize()
{
...
int CropFaceImageComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
    	...
    	// Server adapter creation and publication
		Ice::ObjectAdapterPtr adapterCropFaceImage = communicator()->createObjectAdapter("CropFaceImageComp");
		CropFaceImageI *cropfaceimage = new CropFaceImageI(worker);
		adapterCropFaceImage->add(cropfaceimage, communicator()->stringToIdentity("cropfaceimage"));

		adapterCropFaceImage->activate();
